            ...
            : students.map(student => (
              <button key={student.id}
                ...
                className={"w-full flex items-center gap-2.5 px-4 py-2.5 text-left transition-colors " +
                  (selectedStudentId === student.id ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-100')}>
...
function FeedbackList({ studentId, studentName }: { studentId: string; studentName: string }) {
  ...
  return (
    <div className="p-6 max-w-xl space-y-2.5">
      <div className="mb-5">
...
function FeedbackCard({ classItem, studentId, studentName, isOpen, onToggle }: {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
      <button className="w-full flex items-center justify-between px-5 py-3.5 hover:bg-gray-50 transition-colors" onClick={onToggle}>
        ...
        <div className="border-t border-gray-50 px-5 py-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={generateAI} disabled={generating}
              className="flex items-center gap-1 text-xs px-2.5 py-1 bg-violet-50 text-violet-600 border border-violet-200 rounded-lg hover:bg-violet-100 disabled:opacity-50 transition-colors font-medium">
