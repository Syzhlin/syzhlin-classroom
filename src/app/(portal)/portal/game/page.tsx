  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="w-full">
        ...
        <div className="w-full h-2 rounded-full overflow-hidden" style={{backgroundColor: 'var(--sz-bg-pastel)'}}>
          <div
            className="h-full rounded-full transition-all"
            style={{backgroundColor: 'var(--sz-blue-soft)', width: `${((idx) / cards.length) * 100}%`}}
          />
        </div>
      </div>
      ...
      {flipped ? (
        <div className="flex gap-3 w-full">
          <button
            ...
            className="flex-1 py-3.5 rounded-2xl text-sm font-semibold" style={{backgroundColor: 'var(--sz-bg-pastel)', color: 'var(--sz-text-muted)'}}
          >
            😅 몰랐어요
          </button>
          <button
            onClick={() => handleKnow(true)}
            className="flex-1 py-3.5 text-white rounded-2xl text-sm font-semibold" style={{backgroundColor: 'var(--sz-blue-soft)'}}
          >
            ✅ 알았어요!
          </button>
        </div>
      ...
      <div className="w-full">
        ...
        <div className="w-full h-2 rounded-full overflow-hidden" style={{backgroundColor: 'var(--sz-bg-pastel)'}}>
          <div className="h-full rounded-full transition-all"
            style={{backgroundColor: 'var(--sz-blue-soft)', width: `${(idx / questions.length) * 100}%`}} />
        </div>
      </div>

      {/* 문제 카드 */}
      <div className="w-full rounded-3xl p-8 flex flex-col items-center gap-3 transition-colors"
        style={feedback === 'correct' ? {backgroundColor: 'var(--sz-sage-pale)'} :
               feedback === 'wrong'   ? {backgroundColor: 'var(--sz-pink-pale)'} :
    ...
    <div className="flex flex-col items-center gap-6 py-6">
      ...
      <div className="relative w-28 h-28">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
          ...
          <circle cx="50" cy="50" r="40" fill="none" stroke="var(--sz-blue-soft)" strokeWidth="10"
            strokeDasharray={`${pct * 2.51} 251`} strokeLinecap="round" />
        </svg>
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-2xl font-bold" style={{color: 'var(--sz-blue-soft)'}}>{pct}%</span>
        </div>
      </div>
      ...
      <button onClick={onRetry}
        className="flex items-center gap-2 px-6 py-3 text-white rounded-2xl font-semibold" style={{backgroundColor: 'var(--sz-blue-soft)'}}>
...
export default function GamePage() {
  ...
  return (
    <div className="max-w-sm mx-auto px-4 py-6 space-y-4">
      <div>
        <h1 className="text-xl font-bold" style={{color: 'var(--sz-text-deep)'}}>게임 🎮</h1>
        <p className="text-sm mt-0.5" style={{color: 'var(--sz-text-muted)'}}>영어 단어를 게임으로 익혀요!</p>
      </div>
